...
class CollateralTab(QWidget):
    ...
    def create_form_toggle(self):
        """Create form toggle section with styled buttons"""
        toggle_group = QGroupBox("📝 Form Selection")
        toggle_group.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        
        # Main layout for the group box
        toggle_main_layout = QVBoxLayout(toggle_group)
        toggle_main_layout.setContentsMargins(
            AppStyles.PADDING_SMALL,
            AppStyles.PADDING_SMALL,
            AppStyles.PADDING_SMALL,
            AppStyles.PADDING_SMALL
        )
        toggle_main_layout.setSpacing(AppStyles.SPACING_SMALL)

        # Create button group for exclusive selection
        self.form_toggle_group = QButtonGroup(self)
        
        # Basic Form Button
        self.basic_form_btn = QPushButton("📋 Basic Form")
        self.basic_form_btn.setCheckable(True)
        self.basic_form_btn.setChecked(True)  # Default selection
        self.basic_form_btn.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
        self.basic_form_btn.setMinimumWidth(150)
        self.basic_form_btn.setMaximumWidth(200)
        self.basic_form_btn.clicked.connect(lambda: self.switch_form("basic"))
        
        # Extended Form Button
        self.extended_form_btn = QPushButton("📊 Extended Form")
        self.extended_form_btn.setCheckable(True)
        self.extended_form_btn.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Fixed)
        self.extended_form_btn.setMinimumWidth(150)
        self.extended_form_btn.setMaximumWidth(200)
        self.extended_form_btn.clicked.connect(lambda: self.switch_form("extended"))
        
        # Add buttons to button group
        self.form_toggle_group.addButton(self.basic_form_btn, 0)
        self.form_toggle_group.addButton(self.extended_form_btn, 1)
        
        # Button layout - horizontal with center alignment
        button_layout = QHBoxLayout()
        button_layout.addStretch()
        button_layout.addWidget(self.basic_form_btn)
        button_layout.addSpacing(AppStyles.SPACING_MEDIUM)
        button_layout.addWidget(self.extended_form_btn)
        button_layout.addStretch()
        
        # Info label
        info_label = QLabel("💡 Switch between Basic and Extended collateral forms")
        info_label.setAlignment(Qt.AlignCenter)
        info_label.setStyleSheet(f"""
            color: {AppStyles.TEXT_SECONDARY};
            font-style: italic;
            font-size: {AppStyles.FONT_SMALL}px;
            padding: 5px;
        """)
        
        # Add everything to main layout
        toggle_main_layout.addLayout(button_layout)
        toggle_main_layout.addWidget(info_label)
        
        self.main_layout.addWidget(toggle_group)

    def switch_form(self, form_type):
        """Switch between basic and extended forms"""
        self.current_form_mode = form_type
        
        if form_type == "basic":
            self.stack.setCurrentWidget(self.basic_widget)
            self.basic_form_btn.setChecked(True)
            self.extended_form_btn.setChecked(False)
        else:
            self.stack.setCurrentWidget(self.extended_widget)
            self.extended_form_btn.setChecked(True)
            self.basic_form_btn.setChecked(False)
    ...
    def update_toggle_button_styles(self):
        ...
        # Define styles for toggle buttons specifically
        active_style = f"""
            QPushButton[toggleActive="true"] {{
            ...
            QPushButton[toggleActive="true"]:hover {{
        ...
        inactive_style = f"""
            QPushButton[toggleActive="false"] {{
            ...
            QPushButton[toggleActive="false"]:hover {{
        ...
        # Set properties and apply styles
        if self.current_form_mode == "basic":
            self.basic_form_btn.setProperty("toggleActive", "true")
            self.extended_form_btn.setProperty("toggleActive", "false")
        else:
            self.extended_form_btn.setProperty("toggleActive", "true")
            self.basic_form_btn.setProperty("toggleActive", "false")
        
        # Apply the styles
        self.basic_form_btn.setStyleSheet(active_style + inactive_style)
        self.extended_form_btn.setStyleSheet(active_style + inactive_style)
        
        # Force style refresh
        self.basic_form_btn.style().unpolish(self.basic_form_btn)
        self.basic_form_btn.style().polish(self.basic_form_btn)
        self.extended_form_btn.style().unpolish(self.extended_form_btn)
        self.extended_form_btn.style().polish(self.extended_form_btn)
    ...
    def create_basic_form(self):
        ...
        self.share_amount = self.create_responsive_line_edit()
        ...
        self.share_amount.editingFinished.connect(self.update_total_saving)

        # Add fields to form
        form_layout.addRow("मासिक बचत:", self.monthly_saving)
        form_layout.addRow("बाल बचत:", self.child_saving)
        form_layout.addRow("सेयर रकम:", self.share_amount)
    ...
    def update_total_saving(self):
        ...
        try:
            ...
            share = float(self.share_amount.text() or 0)
            total = monthly + child + share
    ...
    def save_basic_collateral_data(self):
        ...
        try:
            ...
            data = {
                ...
                'share_amount': convert_to_nepali_digits(share),
